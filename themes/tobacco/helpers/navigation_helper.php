...
{
	function get_tree_navigation($items, $id = NULL, $class = NULL, $first_class = NULL, $last_class = NULL, $is_child=false)
	{
		// HTML Attributes
		$id = ( ! is_null($id) ) ? ' id="' . $id . '" ' : '';
		...
		foreach($items as $key => $page)
		{
            ...
            if ($key !== 'articles')
            {
                ...
                $tree .= '<li'.$class . '>'.
					'<a'.$class. $attr .' href="' . (($page['has_url'] != 0 && empty($page['children'])) ? $page['absolute_url'] : 'javascript:') . '">'.$title.
					(($page['has_url'] != 0 && empty($page['children'])) ? '' : ' <b class="caret"></b>') .
					'</a>';

                if (!empty($page['children']))
					$tree .= get_tree_navigation( $page['children'], null, null, null, null, true);
