 ...
 *
 */
if( ! function_exists('get_tree_navigation_canvas'))
{
	function get_tree_navigation_canvas($items, $id = NULL, $class = NULL, $first_class = NULL, $last_class = NULL, $is_child=false)
	{
		// HTML Attributes
		$id = ( ! is_null($id) ) ? ' id="' . $id . '" ' : '';
        ...
        if ($is_child==true)
        {
            $for_child = ' uk-nav uk-nav-navbar';
        }
		$class = ( ! is_null($class) ) ? ' class="' . $class . $for_child . '" ' : ' class="' . $for_child . '"';


		$tree = '<ul data-uk-nav="" ' . $id . $class . '>';
		...
		foreach($items as $key => $page)
		{
            ...
            if ($key !== 'articles')
            {
                ...
                $tree .= '<li'.$class. $attr . '><a'.' href="' . (($page['has_url'] != 0 && empty($page['children'])) ? $page['absolute_url'] : '#') . '">'.$title. '</a>';

                if (!empty($page['children']))
                {
                    //$tree .= '<div class="uk-dropdown uk-dropdown-navbar">';
                    $tree .= get_tree_navigation( $page['children'], null, 'uk-nav-sub', null, null, true);
                    //$tree .= '</div>';
                }
